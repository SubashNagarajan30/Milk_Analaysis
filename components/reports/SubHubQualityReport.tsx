...
interface SubHubQualityReportProps {
  hubId: string;
  startDate: string;
  endDate: string;
}
...
const SubHubQualityReport: React.FC<SubHubQualityReportProps> = ({ hubId, startDate, endDate }) => {
  const { getRecordsForHub, getHubById } = useData();
  ...
  }, [hub, startDate, endDate, getRecordsForHub]);
  ...
  const renderRetestSummaryCard = (title: string, diffVal: number, unit: string = '') => {
    ...
    const description = isPositive 
      ...
      : isNegative 
        ? 'Main Hub recorded lower values overall' 
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <h3 className="text-xl font-semibold text-gray-800 mb-4">Quality Control Report</h3>
        <p className="text-sm text-gray-500 mb-6">Comparison of your entries with main hub re-tests from <span className="font-semibold">{startDate}</span> to <span className="font-semibold">{endDate}</span></p>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th scope="col" className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider text-blue-600">My Qty</th>
              ...
              <th scope="col" className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider text-blue-600">My Fat</th>
              ...
              <th scope="col" className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider text-blue-600">My SNF</th>
...
export default SubHubQualityReport;
